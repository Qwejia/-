...
Page({
  ...
  async deleteRecord(e) {
    ...
    try {
      // 使用网络管理器调用云函数
      await networkManager.callCloudFunction({
      ...
      console.error('云函数调用失败，使用本地删除：', error);
      // 使用本地数据删除
  ...
  async onInsertConfirm() {
    ...
    try {
      ...
      console.error('插入失败，使用本地插入：', error);
      // 使用本地数据插入
      const newRecord = {
  ...
  async getOpenId() {
    ...
    try {
      // 使用网络管理器调用云函数
      const result = await networkManager.callCloudFunction({
        name: "quickstartFunctions",
        data: {
          type: "getOpenId",
        },
      ...
      this.setData({
        haveGetOpenId: true,
        openId: result.openid || `mock-openid-${Date.now()}`,
      });
    } catch (error) {
      console.error('获取OpenId失败：', error);
      const { errMsg } = error;
      if (errMsg && errMsg.includes("Environment not found")) {
        this.setData({
          showTip: true,
          title: "云开发环境未找到",
          content:
      ...
      if (errMsg && errMsg.includes("FunctionName parameter could not be found")) {
        this.setData({
          showTip: true,
          title: "请上传云函数",
          content:
  ...
  async getRecord() {
    ...
    try {
      ...
      console.error('云函数调用失败，使用模拟数据：', error);
      // 使用模拟数据
      this.setData({
        haveGetRecord: true,
        record: [
  ...
  async uploadImg() {
    ...
    try {
      ...
      // 将图片上传至云存储空间
      const uploadResult = await networkManager.uploadFile({
        // 指定上传到的云路径
